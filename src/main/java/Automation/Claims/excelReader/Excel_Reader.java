...
public class Excel_Reader {
	...
	public String[][] GetCellDataFromTill(String sheetName, int startRow, int endRow) {
		String dataset[][]= null;
		XSSFSheet sheet= workbook.getSheet(sheetName);
		
		if(!(startRow < 1) && !(endRow > sheet.getLastRowNum())) {
			try {
				
				startRow= sheet.getRow(startRow).getRowNum();
				endRow= sheet.getRow(endRow).getRowNum()+ 1;
				int totalCols= sheet.getRow(0).getLastCellNum();
				
				dataset= new String[endRow-1][totalCols];	
